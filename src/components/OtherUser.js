...
export const OtherUser = () => {
  ...
  if (!otherUser) return <div id="loading-container">
              ...
              <TabPanel>
                {(otherUser.recAreaWishList.length || otherUser.campgroundWishList.length) ?
                  <div className="wish-list">
                    {otherUser.recAreaWishList.map((recArea, i) => <Link to={{ pathname: `/recareas/${recArea._id}`, state: { recAreaId: recArea._id } }} key={i}><SiteList site={recArea} /></Link>)}
                    {otherUser.campgroundWishList.map((campground, i) => <Link to={`/campgrounds/${campground._id}`} key={i}><SiteList site={campground} /></Link>)}
                  </div> :
              ...
              <TabPanel>
                {(otherUser.recAreasVisited.length || otherUser.campgroundsVisited.length) ?
                  <div className="visited">
                    {otherUser.recAreasVisited.map((recArea, i) => <Link to={{ pathname: `/recareas/${recArea._id}`, state: { recAreaId: recArea._id } }} key={i}><SiteList key={i} site={recArea} /></Link>)}
                    {otherUser.campgroundsVisited.map((campground, i) => <Link to={`/campgrounds/${campground._id}`} key={i}><SiteList key={i} site={campground} /></Link>)}
                  </div> :
            ...
            {(otherUser.recAreaReviews.length || otherUser.campgroundReviews.length) ?
              <div className="user-reviews">
                <div className="user-rec-area-reviews">
                  {otherUser.recAreaReviews.map((review, i) => <ReviewListItem key={i} review={review} enableComments={false} displayAvatar={false} />)}
                </div>
                <div className="campground-reviews">
                  {otherUser.campgroundReviews.map((review, i) => <ReviewListItem key={i} review={review} enableComments={false} displayAvatar={false} />)}
                </div>
              </div> :
